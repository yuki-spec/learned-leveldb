...
int main(int argc, char *argv[]) {
    ...
    for (size_t outer = 0; outer < num_pairs.size(); ++outer) {
        ...
        for (size_t iteration = 0; iteration < num_iteration; ++iteration) {
            ...
            adgMod::block_restart_interval = options.block_restart_interval = adgMod::MOD ? 1 : adgMod::block_restart_interval;
            ...
            for (int cut = 0; cut < cut_size; ++cut) {
                for (int i = keys.size() * cut / cut_size; i < keys.size() * (cut + 1) / cut_size; ++i) {
                    string value = generate_value(0);
                    status = db->Put(write_options, keys[i], value);
                    assert(status.ok() && "File Put Error");
                }
            ...
            adgMod::db->WaitForBackground();
            delete db;
            system("sync; echo 1 | sudo tee /proc/sys/vm/drop_caches");
            ...
            for (int i = 0; i < num_gets; ++i) {
                string value;
                const string& key = keys[uniform_dist_file(e1)];
